...
export const useProductFormStore = create<FormState>((set, get) => ({
    ...
    validate: () => {
        ...
        if (!values.name) newErrors.name = 'نام محصول را وارد نمایید.' //'Name is required';
        if (values.accessoriesCharge == null || isNaN(parseFloat(values.accessoriesCharge))) newErrors.accessoriesCharge = 'هزینه اکسسوری را وارد نمایید.' //'Valid accessories charge is required';
        if (values.karat == null || isNaN(parseFloat(values.karat))) newErrors.karat = 'عیار محصول را وارد نمایید.' //'Valid karat is required';
        if (values.weight == null || isNaN(parseFloat(values.weight))) newErrors.weight = 'وزن محصول را وارد نمایید.' //'Valid weight is required';
        if (!values.type) newErrors.type = 'نوع محصول را انتخاب نمایید.' //'Type is required';
        if (!values.subType) newErrors.subType = 'نوع جنس محصول را انتخاب نمایید.' //'SubType is required';
        if (values.inventoryItem === undefined || values.inventoryItem === null) newErrors.inventoryItem = 'آیا محصول ویترینی می باشد؟' //'Inventory Item Status is required';
        if (values.quantity == null || isNaN(parseInt(values.quantity))) newErrors.quantity = 'تعداد محصول را وارد نمایید.' //'Valid quantity is required';
        if (values.makingChargeBuy == null || isNaN(parseFloat(values.makingChargeBuy))) newErrors.makingChargeBuy = 'درصد اجرت خرید را وارد نمایید.' //'Valid making charge buy is required';
        if (values.makingChargeSell == null || isNaN(parseFloat(values.makingChargeSell))) newErrors.makingChargeSell = 'درصد اجرت فروش را وارد نمایید.' //'Valid making charge sell is required';
        if (Number(values.makingChargeSell) < Number(values.makingChargeBuy)) newErrors.makingChargeSell = 'درثد اجرت فروش نمی تواند کمتر از اجرت خرید باشد.' //'It is cant be less than making charge buy';
        if (values.vat == null || isNaN(parseFloat(values.vat))) newErrors.vat = 'درصد مالیات را وارد نمایید' //'Valid VAT is required';
        if (values.profit == null || isNaN(parseFloat(values.profit))) newErrors.profit = 'درصد سود را وارد نمایید' //'Valid PROFIT is required';
        if (values.tags.length === 0) newErrors.tags = 'حداقل بایستی یک تگ را انتخاب نمایید.' //'At least one tag is required';
        if (values.photos.reduce((p, c) => p + c.size, 0) > 400 * 1024 * 1024) newErrors.photos = `حد اکثر 400 مگابایت می تولنید آپلود نمایید - حجم پیوستی ${Math.round(values.photos.reduce((p, c) => p + c.size, 0) / (1024 * 1024) * 100) / 100} مگا بایت می باشد.` //`At Max state, 400 MB have allowed to be attached for each product. your files size is: ${Math.round(values.photos.reduce((p, c) => p + c.size, 0) / (1024 * 1024) * 100) / 100}MB`;
        if (values.photos.length > 12) newErrors.photos = 'به ازای هر محصول حد اکثر 12 تصویر می توانید آپلود کنید.' //'At Max state, 12 photos allowed to be attached for each product.';
        if (values.photos.length === 0) newErrors.photos = 'حداقل یک تصویر انتخاب نمایید.' //'At least one photo is required';
        if (values.previews.length !== values.photos.length) newErrors.previews = 'مشکل در همخوانی تعداد تصاویر پیوستی و پیش نمایش های ایجاد شده' //'Previews must match photos count';
